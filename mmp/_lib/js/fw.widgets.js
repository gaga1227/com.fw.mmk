...
function initSharingButtons(){
	...
	//exit if no dom elems, or non touch
	if ( !$links.length || !$btnShare.length ) return false;
	//attach behavior
	$btnShare.on('click', function(e){
		e.preventDefault();
		if (isTouch) $links.hasClass(activeCls) ? $links.removeClass(activeCls) : $links.addClass(activeCls);
	});
}