/* ------------------------------------------------------------------------------ */
/* common - debug - Avoid 'console' errors in browsers that lack a console. */
/* ------------------------------------------------------------------------------ */
(function() {
    var method,
		methods = [
			'assert', 'clear', 'count', 'debug', 'dir', 'dirxml', 'error',
			'exception', 'group', 'groupCollapsed', 'groupEnd', 'info', 'log',
			'markTimeline', 'profile', 'profileEnd', 'table', 'time', 'timeEnd',
			'timeStamp', 'trace', 'warn'
   		],
    	length = methods.length,
    	console = (window.console = window.console || {}),
		noop = function () {};
	
    while (length--) {
        method = methods[length];
        if (!console[method]) {
            console[method] = noop;
        }
    }
}());
/* ------------------------------------------------------------------------------ */
/* common - debug - displayDebugInfo */
/* ------------------------------------------------------------------------------ */
function displayDebugInfo(debug){
	
	//vars
	var $debug = $(debug),
		$html = $('html'),
		on = '',
		toggle = function() {
			if ( on == 'true' ) {
				$debug.css('opacity', '1');
				$html.addClass('debug');	
			} else {
				$debug.css('opacity', '0');
				$html.removeClass('debug');
			}
		},
		update = function(){
			$debug.attr('data-width',$(window).width());
			on = $debug.attr('data-on');
			toggle();
		};
	
	//init and bind event
	$debug.on('click', function(e){
		e.preventDefault();
		var state = '';
		( on == 'true' ) ? state = 'false' : state = 'true';
		$debug.attr('data-on', state);
		update();
	});
	
	//init
	update();
	$(window).bind('resize', update);
		
}
/* ------------------------------------------------------------------------------ */
/* common - debug - debugSkin */
/* ------------------------------------------------------------------------------ */
function debugSkin(domEle, cssRef, skins){
	
	//exit if no switcher in DOM, or no css ref, or no more than 1 skin
	if ( !$(domEle).length || !$(cssRef).length || skins.length <= 1 ) return false;
	
	//define gloabl obj
	var skinsObj = {};

	//function updateNav
	skinsObj.updateNav = function(skin){
		$.each(this.$navs, function(idx, ele){
			//vars
			var $nav = $(ele),
				navCls = $nav.attr('class'),
				targetNav = (navCls.indexOf(skin) != -1) ? true : false;
			//switch nav
			targetNav ? $nav.removeClass('hidden') : $nav.addClass('hidden');
		});
	}

	//function updateImgs
	skinsObj.updateImgs = function(skin){
		$.each(this.$imgs, function(idx, ele){
			//vars
			var $img = $(ele),
				imgSRC = $img.attr('src'),
				isSame = (imgSRC.indexOf(skin) != -1) ? true : false,
				updateEndIdx, updateStartIdx, newImgSRC;
			//exit if new skin == old skin
			if (isSame) return false
			//update new skin
			updateEndIdx = imgSRC.lastIndexOf('/'),
			updateStartIdx = imgSRC.substr(0, updateEndIdx).lastIndexOf('/'),
			newImgSRC = imgSRC.substr(0, updateStartIdx+1) + skin + imgSRC.substr(updateEndIdx),
			$img.attr('src', newImgSRC);
			//console.log('old:', imgSRC);
			//console.log('new:', newImgSRC);
		});
	}

	//function updateSkin
	skinsObj.updateSkin = function(id){
		//vars
		var skin = this.skins[id],
			oldSkinPath = this.$cssRef.attr('href'),
			newSkinPath = '',
			pathEndIdx = (oldSkinPath.indexOf('skin') != -1) ? oldSkinPath.indexOf('skin') + 4 : undefined;
		//exit if no idx or skin found
		if ( !pathEndIdx || !skin ) return false
		//update skin css ref
		newSkinPath = oldSkinPath.substring(0, pathEndIdx) + '-' + skin + '.css'; 
		if (newSkinPath != oldSkinPath) {
			this.$cssRef.attr( 'href', newSkinPath);
			respond.update();
			//console.log('cssRef updated:', newSkinPath);
		} else { 
			//console.log('same skin id, no update');
		}
		//update img
		this.updateImgs(skin);
		//update body
		$('body').attr('class', skin);
		//update nav
		//this.updateNav(skin);
	}
	
	//function updateNextSkinID
	skinsObj.updateNextSkinID = function(){
		this.skinCurrent = (this.skinCurrent >= 0) ? this.skinCurrent + 1 : 0;
		this.skinCurrent = (this.skinCurrent == this.skins.length) ? 0 : this.skinCurrent;
		return this.skinCurrent;
	}
	
	//function init
	skinsObj.init = function(){
		//update info
		this.$switcher = $(domEle);
		this.$cssRef = $(cssRef);
		this.$navs = $('#nav .navItem');
		this.$imgs = $('img.skin');
		this.skins = skins;
		this.skinOriginal = this.$cssRef.attr('href');
		this.skinDefault = this.$switcher.attr('data-default') ? parseInt(this.$switcher.attr('data-default'), 10) : undefined;
		this.skinCurrent = this.skinDefault;
		//switch to default skin
		this.updateSkin( this.skinCurrent );
		//bind button events
		this.$switcher.on('click', function(e){
			e.preventDefault();
			skinsObj.updateSkin( skinsObj.updateNextSkinID() );
			//console.log( skinsObj.skinCurrent );
		});
	}
	
	//init obj
	skinsObj.init();
	
	//return gloabal obj
	return skinsObj;
	
}
/* ------------------------------------------------------------------------------ */
/* common - get Platform */
/* ------------------------------------------------------------------------------ */
var Platform = new function(){
	//detecting functions
	function checkPlatform(os) { return (navigator.userAgent.toLowerCase().indexOf(os.toLowerCase())>=0); }
	function checkEvent(e) { return (e in document.documentElement); }
	//add properties
	this.iPhone = checkPlatform('iPhone');
	this.iPad = checkPlatform('iPad');
	this.iPod = checkPlatform('iPod');
	this.iOS = this.iPhone||this.iPad||this.iPod;
	this.android = checkPlatform('android');
	this.touchOS = checkEvent('ontouchstart');
	this.addDOMClass = function(){
		var $html = $('html'),
			cls = '';
		if ( this.iPhone )	cls = 'iPhone';
		if ( this.iPad )	cls = 'iPad';
		if ( this.iPod )	cls = 'iPod';
		if ( this.iOS )		cls = 'iOS';
		if ( this.android )	cls = 'android';
		$html.addClass(cls);
	}
	this.debugLog = function(){
		console.log('iPhone: '+this.iPhone);
		console.log('iPad: '+this.iPad);
		console.log('iPod: '+this.iPod);
		console.log('iOS: '+this.iOS);
		console.log('android: '+this.android);
		console.log('touchOS: '+this.touchOS);
	}
	//css3 transition end event
	this.transEndEventNames = {
		'WebkitTransition' : 'webkitTransitionEnd',
		'MozTransition'    : 'transitionend',
		'OTransition'      : 'oTransitionEnd',
		'msTransition'     : 'MSTransitionEnd',
		'transition'       : 'transitionend'
	}
	this.transEndEventName = this.transEndEventNames[ Modernizr.prefixed('transition') ];
	
	//return self
	return this;
}
/* ------------------------------------------------------------------------------ */
/* common - mqStates */
/* ------------------------------------------------------------------------------ */
var mqStates = {
	max960:	'only screen and (max-width:960px)',
	max800:	'only screen and (max-width:800px)',
	max640:	'only screen and (max-width:640px)',
	max500:	'only screen and (max-width:500px)',
	max400:	'only screen and (max-width:400px)',
	max320:	'only screen and (max-width:320px)'
}
/* ------------------------------------------------------------------------------ */
/* common - insert First and Last Child */
/* ------------------------------------------------------------------------------ */
function insertFirstLastChild(selection) {
	var $tgts = $(selection);//cache selection
	if (!$tgts.length) return false;//cancel if no target found
	$.each($tgts,function(idx,ele){//go through all selected items
		var $ele = $(ele),//cache current item
			$fstChild = $ele.find('> :first-child'),//find and cache first-child
			$lstChild = $ele.find('> :last-child');//find and cache last-child
		//add class if not already
		if (!$fstChild.hasClass('first-child')) { $fstChild.addClass('first-child'); }
		if (!$lstChild.hasClass('last-child')) { $lstChild.addClass('last-child'); }
	});	
}
/* ------------------------------------------------------------------------------ */
/* common - initCSS3PIE */
/* ------------------------------------------------------------------------------ */
function initCSS3PIE() {
	var oldIE = $('html').hasClass('oldie'),
		triggerCLS = 'css3pie';
	if ( oldIE && window.PIE ) {
		$.each( $('.' + triggerCLS), function(idx, ele){
			PIE.attach(ele);
		});
	} else {
		return false;	
	}
}
/* ------------------------------------------------------------------------------ */
/* common - cssAnim (working specifically with animate.css) */
/* ------------------------------------------------------------------------------ */
function cssAnim(target,anim) {
	if ( !Modernizr.cssanimations || !target.length || !anim ) return false;
	var $target = target,
		animCls = anim,
		scopeCls = 'animated',
		cleanTarget = function(){ $target.removeClass(scopeCls).removeClass(animCls); },
		updateTarget = function(){ cleanTarget(); $target.addClass(scopeCls).addClass(animCls); },
		delay;
	updateTarget();
	delay = window.setTimeout( function(){ cleanTarget() }, 1300 );
}
/* ------------------------------------------------------------------------------ */
/* common - initBtnScroll */
/* ------------------------------------------------------------------------------ */
function initBtnScroll( cls ) {
	//vars
	var btnScrollsCls = cls || '.btnScroll',
		$btnScrolls = $(btnScrollsCls),
		defaultTarget = '#container',
		defaultSpeed = 1000;
	
	//exit if no btnScrolls or scrollTo not loaded
	if ( !$btnScrolls.length || !$().scrollTo ) return false;
	
	//process each btnScrolls instance
	$.each($btnScrolls, function(idx,ele){
		//vars
		var $ele = $(ele);
		//attach behavior to instance
		$ele.on('click',function(e){
			e.preventDefault();
			var target = $ele.attr('data-target') || defaultTarget,
				speed = Number($ele.attr('data-speed') || defaultSpeed);
			console.log(target, speed);
			$.scrollTo( $(target), speed, {axis:'y'} );
		});
	});	
}




















